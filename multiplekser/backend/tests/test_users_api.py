...
from tests.conftest import ELEKTRYK_PASSWORD


def test_list_users_wymaga_roli_admin(client, elektryk_headers):
    r = client.get("/users", headers=elektryk_headers)
    assert r.status_code == 403


def test_list_users_dziala_dla_admina(client, admin_headers, admin_user, elektryk_user):
    r = client.get("/users", headers=admin_headers)
    assert r.status_code == 200
    emails = {u["email"] for u in r.json()}
    assert {admin_user.email, elektryk_user.email} <= emails


def test_create_user_wymaga_roli_admin(client, elektryk_headers):
    r = client.post(
        "/users", json={"email": "nowy@test.local", "password": "haslo1234"}, headers=elektryk_headers
    )
    assert r.status_code == 403
...
def test_update_user_sukces(client, admin_headers, elektryk_user):
    r = client.put(
        f"/users/{elektryk_user.id}",
        json={
            "email": elektryk_user.email, "rola": "elektryk",
...
def test_update_user_dezaktywacja(client, admin_headers, elektryk_user):
    r = client.put(
        f"/users/{elektryk_user.id}",
        json={"email": elektryk_user.email, "rola": "elektryk", "magazyny_dostepne": [], "active": False},
        headers=admin_headers,
    )
    assert r.status_code == 200
...
def test_update_user_nieistniejacy_zwraca_404(client, admin_headers):
    r = client.put(
        ...
        json={"email": "x@test.local", "rola": "elektryk", "magazyny_dostepne": [], "active": True},
        headers=admin_headers,
    )
    assert r.status_code == 404


def test_update_user_email_zajety_przez_innego_zwraca_409(client, admin_headers, admin_user, elektryk_user):
    r = client.put(
        f"/users/{elektryk_user.id}",
        json={"email": admin_user.email, "rola": "elektryk", "magazyny_dostepne": [], "active": True},
        headers=admin_headers,
    )
    assert r.status_code == 409


def test_update_user_wymaga_roli_admin(client, elektryk_headers, elektryk_user):
    r = client.put(
        f"/users/{elektryk_user.id}",
        json={"email": elektryk_user.email, "rola": "elektryk", "magazyny_dostepne": [], "active": True},
...
def test_reset_password_sukces_i_nowe_haslo_dziala(client, admin_headers, elektryk_user):
    r = client.post(
        f"/users/{elektryk_user.id}/reset-password",
        json={"new_password": "nowe-haslo-123"},
        headers=admin_headers,
    )
    assert r.status_code == 200

    login = client.post("/auth/token", data={"username": elektryk_user.email, "password": "nowe-haslo-123"})
    assert login.status_code == 200

    old_login = client.post("/auth/token", data={"username": elektryk_user.email, "password": ELEKTRYK_PASSWORD})
    assert old_login.status_code == 401


def test_reset_password_wymaga_roli_admin(client, elektryk_headers, elektryk_user):
    r = client.post(
        f"/users/{elektryk_user.id}/reset-password",
        json={"new_password": "nowe-haslo-123"}, headers=elektryk_headers,
    )
    assert r.status_code == 403
